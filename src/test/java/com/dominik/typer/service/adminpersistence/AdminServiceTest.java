...
class AdminServiceTest {
    ...
    void givenAdminUser_whenIsAdmin_thenReturnTrue() {
        //given
        UserEntity userEntity = UserEntity.builder().username("Dominik").userType(UserRole.ADMIN).build();
    ...
    @Test
    void givenNonAdminUser_whenIsAdmin_thenThrowException() {
        //given
        UserEntity userEntity = UserEntity.builder().username("Dominik").userType(UserRole.USER).build();
        given(userRepository.getByUsername("Dominik")).willReturn(Optional.ofNullable(userEntity));
        //when
